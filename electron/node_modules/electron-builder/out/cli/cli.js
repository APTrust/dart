...
let rebuildAppNativeCode = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (args) {
        ...
        (_builderUtil || _load_builderUtil()).log.info({ platform: args.platform, arch: args.arch }, "executing node-gyp rebuild");
...
}).demandOption("publisher"), wrap(argv => (0, (_createSelfSignedCert || _load_createSelfSignedCert()).createSelfSignedCert)(argv.publisher))).command("start", "Run application in a development mode using electron-webpack", yargs => yargs, wrap(() => (0, (_start || _load_start()).start)())).help().epilog(`See ${(_chalk || _load_chalk()).default.underline("https://electron.build")} for more documentation.`).strict().recommendCommands().argv;
function wrap(task) {
    return args => {
        checkIsOutdated();
        (0, (_readConfigFile || _load_readConfigFile()).loadEnv)(_path.join(process.cwd(), "electron-builder.env")).then(() => task(args)).catch(error => {
            console.error((_chalk || _load_chalk()).default.red(error instanceof (_electronBuilderLib || _load_electronBuilderLib()).MisConfigurationError ? error.message : (error.stack || error).toString()));
            process.exitCode = 1;
        });
    };
}
function checkIsOutdated() {
    ...
    (0, (_fsExtraP || _load_fsExtraP()).readJson)(_path.join(__dirname, "..", "..", "package.json")).then(it => {
        ...
        if (notifier.update != null) {
            notifier.notify({
                message: `Update available ${(_chalk || _load_chalk()).default.dim(notifier.update.current)}${(_chalk || _load_chalk()).default.reset(" → ")}${(_chalk || _load_chalk()).default.green(notifier.update.latest)} \nRun ${(_chalk || _load_chalk()).default.cyan("yarn upgrade electron-builder")} to update`
            });
        }
    }).catch(e => (_builderUtil || _load_builderUtil()).log.warn({ error: e }, "cannot check updates"));
}