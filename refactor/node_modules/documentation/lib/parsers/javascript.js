...
function parseJavaScript(data, config) {
  ...
  const ast = (0, _parse_to_ast.parseToAst)(data.source);
  const addComment = _addComment.bind(null, visited, commentsByNode);

  return _.flatMap(config.documentExported ? [walkExported] : [walkComments.bind(null, 'leadingComments', true), walkComments.bind(null, 'innerComments', false), walkComments.bind(null, 'trailingComments', false)], fn => fn(ast, data, addComment)).filter(comment => comment && !comment.lends);
}
  ...
  if (!visited.has(key)) {
    ...
    if (includeContext) {
      ...
      if (t.isClassMethod(path) && path.node.kind === 'constructor') {
        ...
        const parentComment = commentsByNode.get(path.parentPath.parentPath.node);
        if (parentComment) {
