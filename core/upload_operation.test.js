...
test('validate()', () => {
    ...
    expect(result1.errors['UploadOperation.destination']).toEqual('Destination is required.');
    expect(result1.errors['UploadOperation.protocol']).toEqual('Protocol is required.');
    expect(result1.errors['UploadOperation.sourceFiles']).toEqual('Specify at least one file or directory to upload.');
    ...
    expect(result2.errors['UploadOperation.destination']).toBeUndefined();
    expect(result2.errors['UploadOperation.protocol']).toBeUndefined();
    expect(result2.errors['UploadOperation.sourceFiles']).toBeUndefined();
});
