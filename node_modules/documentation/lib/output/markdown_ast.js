...
function buildMarkdownAST(comments, config) {
  ...
  function generate(depth, comment) {
    ...
    function paramSection(comment) {
      return comment.params.length > 0 && [u('heading', { depth: depth + 1 }, [u('text', 'Parameters')]), paramList(comment.params)];
    }

    function propertySection(comment) {
      return comment.properties.length > 0 && [u('heading', { depth: depth + 1 }, [u('text', 'Properties')]), propertyList(comment.properties)];
    }
    ...
    function examplesSection(comment) {
      return comment.examples.length > 0 && [u('heading', { depth: depth + 1 }, [u('text', 'Examples')])].concat(comment.examples.reduce(function (memo, example) {
