  ...
  comments.forEach(comment => {
    ...
    if (!path) {
      ...
      if (comment.memberof) {
        ...
        path = comment.memberof.split('.').map(segment => ({ scope: 'static', name: segment }));
      }
    ...
    for (const name in nodes) {
      ...
      for (let i = 0; i < node.comments.length; i++) {
        ...
        if (hasUndefinedParent) {
          const memberOfTag = comment.tags.filter(tag => tag.title === 'memberof')[0];
          const memberOfTagLineNumber = memberOfTag && memberOfTag.lineNumber || 0;
