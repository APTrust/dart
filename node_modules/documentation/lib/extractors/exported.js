    ...
    ExportDeclaration(path) {
      ...
      if (t.isExportNamedDeclaration(path)) {
        ...
        specifiers.forEach(specifier => {
          ...
          let bindingPath;
          if (source) {
            const tmp = findExportDeclaration(dataCache, local, exportKind, filename, source.value);
            bindingPath = tmp.ast;
            specData = tmp.data;
...
function getCachedData(dataCache, filePath) {
  ...
  if (!value) {
    ...
    const ast = (0, _parse_to_ast.parseToAst)(input);
    value = {
      data: {
        file: path,
        source: input
        ...
        for (let i = 0; i < specifiers.length; i++) {
          ...
          if (exported === name) {
            if (source) {
              ...
              const tmp = findExportDeclaration(dataCache, local, exportKind, depPath, source.value);
              rv = tmp.ast;
              data = tmp.data;
              if (!rv) {
