...
func WorkflowNewGet(env *Environment, w http.ResponseWriter, r *http.Request) error {
	...
	form.Field("SerializationFormat").SetSelectChoices(GetOptions(env.DB, "SerializationFormat"))
	form.Field("BagItProfileID").SetSelectChoices(GetOptions(env.DB, "BagItProfile"))
	form.Field("StorageServiceID").SetSelectChoices(GetOptions(env.DB, "StorageService"))
	data["form"] = form
